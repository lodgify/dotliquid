...
namespace DotLiquid.Tests.Tags
{
    ...
    public class StandardTagTests
    {
        ...
        public async Task TestHasABlockWhichDoesNothing()
        {
            ...
            await Helper.AssertTemplateResultAsync("", "{% comment %}comment{% endcomment %}");
            //await Helper.AssertTemplateResult("", "{% comment %} 1 {% comment %} 2 {% endcomment %} 3 {% endcomment %}");

            await Helper.AssertTemplateResultAsync("", "{%comment%}{%blabla%}{%endcomment%}");
            await Helper.AssertTemplateResultAsync("", "{% comment %}{% blabla %}{% endcomment %}");
            await Helper.AssertTemplateResultAsync("", "{%comment%}{% endif %}{%endcomment%}");
            await Helper.AssertTemplateResultAsync("", "{% comment %}{% endwhatever %}{% endcomment %}");
            //await Helper.AssertTemplateResult("", "{% comment %}{% raw %} {{%%%%}}  }} { {% endcomment -%} {% comment {% endraw %} {% endcomment %}");
        ...
        [Test]
        public async Task TestForWithNestedDictionary_DotLiquid22()
        {
            var dictionary = new Dictionary<string, object> { {
                "People",
                new Dictionary<string, object> {
...
{%endfor%}";
        ...
        [Test]
        public async Task TestForElse()
        {
            // parity tests with https://github.com/Shopify/liquid/blob/master/test/integration/tags/for_tag_test.rb
            await Helper.AssertTemplateResultAsync("+++", "{%for item in array%}+{%else%}-{%endfor%}", Hash.FromAnonymousObject(new { array = new[] { 1, 2, 3 } }));
            await Helper.AssertTemplateResultAsync("-", "{%for item in array%}+{%else%}-{%endfor%}", Hash.FromAnonymousObject(new { array = new int[0] }));
            await Helper.AssertTemplateResultAsync("-", "{%for item in array%}+{%else%}-{%endfor%}", Hash.FromAnonymousObject(new { array = (int[])null }));
        }
        ...
        [Test]
        public async Task TestDynamicVariableLimitingLong()
        {
            Hash assigns = Hash.FromAnonymousObject(new { array = new[] { 1, 2, 3, 4, 5, 6, 7, 8, 9, 0 } });
        ...
        {
            Assert.ThrowsAsync<SyntaxException>(async () => await Helper.AssertTemplateResultAsync("", "{% case false %}{% when %}true{% endcase %}", new Hash()));
            Assert.ThrowsAsync<SyntaxException>(async () => await Helper.AssertTemplateResultAsync("", "{% case false %}{% huh %}true{% endcase %}", new Hash()));
        }
        ...
        [Test]
        public async Task TestNestedDictionaries_DotLiquid22()
        {
